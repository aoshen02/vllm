...
def test_replay_overwrites_sampled_at_each_step():
    ...
    trace_token_ids = torch.zeros(
        2, TEST_MAX_TRACE_LEN, dtype=torch.int32, device=DEVICE
    )
...
def test_past_end_of_trace_leaves_sampled_untouched():
    ...
    trace_token_ids = torch.zeros(
        1, TEST_MAX_TRACE_LEN, dtype=torch.int32, device=DEVICE
    )
...
def test_state_end_to_end():
    ...
    state = TraceReplayState(
        max_num_reqs=4, device=torch.device(DEVICE), max_trace_len=8192
    )
...
def test_state_allocates_trace_buffer_lazily():
    state = TraceReplayState(
        max_num_reqs=2, device=torch.device(DEVICE), max_trace_len=8192
    )
...
def test_trace_too_long_rejected():
    state = TraceReplayState(
        max_num_reqs=2, device=torch.device(DEVICE), max_trace_len=8192
    )
    with pytest.raises(ValueError, match="too long"):
        state.add_request(
            0, SamplingParams(trace_decode_token_ids=[0] * (TEST_MAX_TRACE_LEN + 1))
        )
