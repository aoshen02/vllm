...
def test_partial_weight_loaders_survive_direct_start():
    """`functools.partial` loaders (RMSNormTP) have no `__name__`."""
    model = _TinyModel()
    for param in model.parameters():
        param.weight_loader = functools.partial(default_weight_loader)
    start_reload(model, "direct")
    model.load_weights(_payload(model, 3.0))
    finish_reload(model, None)
    _assert_all(model, 3.0)
...
def test_switching_from_layerwise_after_an_incomplete_update():
    """A layerwise update whose checkpoint omitted a tensor leaves that
    parameter's loader wrapped; a direct load through the wrapper would write
    nothing."""
    model = _TinyModel(bias=True)
    record_metadata_for_reloading(model)
    partial = [(n, t) for n, t in _payload(model, 2.0) if n != "b.bias"]
    start_reload(model, "layerwise")
    model.load_weights(partial)
    finish_reload(model, None)

    start_reload(model, "direct")
    model.load_weights(_payload(model, 9.0))
    finish_reload(model, None)
    _assert_all(model, 9.0)


def test_active_layerwise_reload_blocks_direct_start():
    model = _TinyModel()
    record_metadata_for_reloading(model)
    start_reload(model, "layerwise")
    with pytest.raises(RuntimeError, match="layerwise reload"):
        start_reload(model, "direct")
    abort_reload(model)
...
def test_a_rebinding_load_is_caught_at_finish(rebind):
    ...
    so the outcome is checked and the model is left undefined."""
    model = _TinyModel()
...
def test_direct_abort_leaves_the_model_undefined():
    model = _TinyModel()
    start_reload(model, "direct")
    model.a.weight.weight_loader(model.a.weight, torch.full_like(model.a.weight, 5.0))
    abort_reload(model)
    assert torch.equal(model.a.weight, torch.full_like(model.a.weight, 5.0))
    with pytest.raises(RuntimeError, match="restarted"):
        start_reload(model, "direct")


def test_abort_without_start_is_a_no_op():
    abort_reload(_TinyModel())
...
def test_ipc_engine_releases_the_packed_import_when_finish_fails(monkeypatch):
    """A failed direct finish refuses every later update, so the importer's
    replace-on-next-export cleanup never runs."""
    import vllm.model_executor.model_loader.reload as reload_pkg
    from vllm.distributed.weight_transfer.ipc_engine import IPCWeightTransferEngine

    def failing_finish(model, cfg):
        raise RuntimeError("relocated")

    monkeypatch.setattr(reload_pkg, "finish_reload", failing_finish)
    closed = []
    fake = SimpleNamespace(
        model=torch.nn.Module(),
        model_config=None,
        _packed_importer=SimpleNamespace(close=lambda: closed.append(True)),
    )
    with pytest.raises(RuntimeError, match="relocated"):
        IPCWeightTransferEngine.finish_weight_update(fake)
    assert closed == [True]
