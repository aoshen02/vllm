...
def _read_in_child(root: str, handle: dict, result_queue) -> None:
    reader = LocalSharedMemoryArtifactReader(root, handle["store_id"])
    result_queue.put(reader.materialize(handle).tolist())
...
def _make_worker_connector(tmp_path, *, max_bytes: int = 1 << 20):
    ...
    connector.writer = FakeRoutingWriter(slots)
    ...
    connector.namespace = "test-namespace"
    connector.policy_epoch = 0
    connector._states = {}
...
def test_worker_finalizes_kv_keyed_blocks_and_request_tail(tmp_path):
    ...
    handle = output.results[0].handle
    assert handle is not None
    reader = LocalSharedMemoryArtifactReader(str(tmp_path), connector.store.store_id)
    np.testing.assert_array_equal(reader.materialize(handle), logical[:10])
    ...
    assert [segment.get("kv_block_hash") for segment in manifest["segments"][:2]] == [
...
def test_worker_terminal_finalize_requires_prior_full_block_commit(tmp_path):
    ...
    assert output is not None
    assert output.results[0].handle is None
    assert "missing an incremental full-block commit" in (output.results[0].error or "")
    connector.close()
...
def test_worker_excludes_rejected_speculative_rows_after_accepted_end(tmp_path):
    ...
    assert output is not None and output.results[0].handle is not None
    materialized = connector.store.materialize(output.results[0].handle)
...
def test_prefix_blocks_deduplicate_across_samples(tmp_path):
    ...
    first_handle = first_output.results[0].handle
    second_handle = second_output.results[0].handle
    assert first_handle is not None and second_handle is not None
    reader = LocalSharedMemoryArtifactReader(str(tmp_path), connector.store.store_id)
    np.testing.assert_array_equal(reader.materialize(second_handle), logical[:10])
    ...
    assert first_manifest["segments"][:2] == second_manifest["segments"][:2]
    assert len(list(connector.store.blocks_dir.glob("*.bin"))) == 2
...
def test_full_blocks_use_canonical_value_across_execution_shapes(tmp_path):
    ...
    first = _request("request-a", "a" * 32, block_ids)
    first_output = _finalize_request(connector, first)
    assert first_output is not None and first_output.results[0].handle is not None
    ...
    assert second_output is not None and second_output.results[0].handle is not None
    expected = logical[:10].copy()
    expected[8:10] += 1
    np.testing.assert_array_equal(second_output.results[0].routed_experts, expected)
    np.testing.assert_array_equal(
        connector.store.materialize(second_output.results[0].handle), expected
    )
    assert len(list(connector.store.blocks_dir.glob("*.bin"))) == 2
    assert len(list(connector.store.tails_dir.glob("*.bin"))) == 2
    connector.close()
...
def test_unaligned_prompt_start_uses_request_local_head(tmp_path):
    ...
    reader = LocalSharedMemoryArtifactReader(str(tmp_path), connector.store.store_id)
    np.testing.assert_array_equal(
        reader.materialize(output.results[0].handle), logical[3:10]
    )
    manifest = reader.read_manifest(request.artifact_sample_id)
    assert [segment["kind"] for segment in manifest["segments"]] == [
        "tail",
    ...
    assert manifest["segments"][1]["kv_block_hash"] == request.block_hashes[1].hex()
    connector.close()
...
def test_reader_materializes_from_another_process(tmp_path):
    ...
    process = context.Process(
        ...
        args=(str(tmp_path), output.results[0].handle, result_queue),
    )
    process.start()
...
def test_corrupt_payload_is_rejected(tmp_path):
    ...
    block_path = connector.store._path("block", manifest["segments"][0]["object_id"])
    ...
    with pytest.raises(ArtifactCorruptionError, match="checksum"):
        connector.store.materialize(output.results[0].handle)
    connector.close()
...
def test_scheduler_connector_tracks_metadata_and_acknowledgements():
    ...
    result = ArtifactFinalizeResult(
        request_id="request-a",
        artifact_sample_id=sample_id,
        handle={"artifact_sample_id": sample_id},
    )
...
def test_scheduler_releases_terminal_output_only_after_artifact_ack():
    ...
    assert terminal_output.artifact == handle
...
def test_scheduler_fails_request_closed_when_artifact_finalize_fails():
    ...
    assert terminal_output.artifact is None
    scheduler._free_blocks.assert_called_once_with(request)
