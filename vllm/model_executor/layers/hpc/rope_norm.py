...
class HpcRopeNorm(CustomOp, HpcModule):
    ...
    The QK-Norm weights are read straight off the fallback norm modules,
    which the model builds in float32 for this path, so there is no derived
    copy to keep in sync across weight reloads.
    ...
    def _forward_impl(
        ...
        use_norm = self.qk_norm_policy != QkNormPolicy.NONE
        q_norm_weight = self.fallback_qnorm.weight if use_norm else None
        k_norm_weight = self.fallback_knorm.weight if use_norm else None
