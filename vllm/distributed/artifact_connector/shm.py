...
class LocalSharedMemoryArtifactStore(LocalSharedMemoryArtifactReader):
    ...
    def retain_blocks(self, object_ids: list[str]) -> None:
        """Retain existing full blocks while a request references them."""
        retained: list[str] = []
        try:
            for object_id in object_ids:
                ...
                if not self._path("block", object_id).exists():
                    raise FileNotFoundError(
                        f"artifact block does not exist: {object_id}"
                    )
                with self._lock:
    ...
    def gc(self, *, lock_held: bool = False) -> None:
        ...
        for path in self.manifests_dir.glob("*.json"):
            ...
            if not isinstance(fields, dict):
                continue
            for field_manifest in fields.values():
                if not isinstance(field_manifest, dict):
                    continue
                for segment in field_manifest.get("segments", ()):
