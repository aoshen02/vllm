...
| all2all backend | **the default is the broken one** | same DP2xTP2+EP shape, one variable apart. `allgather_reducescatter` leaks in every round with graphs on (103 rounds) *and* under `--enforce-eager` (33 rounds). Four independent all2all implementations do not: `deepep_high_throughput` 31 rounds eager, `deepep_low_latency` 35 eager and **68 with graphs on**, `deepep_v2` 9 eager and 55 with graphs, `nixl_ep` 55 with graphs — every one of them 0 differing, and both graph arms separately confirmed that capture really happened rather than silently degrading to eager. The strongest arm runs the full 43-layer model: `deepep_low_latency`, DP2xTP2+EP, graphs on, 17 rounds, 0 differing. Two upstream bugs surfaced on the way and are fixed on their own branches: the HT cudagraph guard is unreachable whenever breakable cudagraph pins the compilation mode to NONE (which is exactly what DeepSeek-V4, Kimi K3, MiniMax-M3 and Inkling do), and the DeepEP v2 decode path builds a zero-length `ExpertTokensMetadata` out of the empty host-side count that its own `do_cpu_sync=False` guarantees |
